...
function AES ( options ) {
    ...
    this.heap = util._heap_init( Uint8Array, options ).subarray( AES_asm.HEAP_DATA );
...
function AES_set_key ( key ) {
    if ( key !== undefined ) {
        if ( util.is_buffer(key) || util.is_bytes(key) ) {
            key = new Uint8Array(key);
        }
        else if ( util.is_string(key) ) {
            key = util.string_to_bytes(key);
        }
        else {
...
function AES_set_iv ( iv ) {
    if ( iv !== undefined ) {
        if ( util.is_buffer(iv) || util.is_bytes(iv) ) {
            iv = new Uint8Array(iv);
        }
        else if ( util.is_string(iv) ) {
            iv = util.string_to_bytes(iv);
        }
        else {
...
function AES_Encrypt_process ( data ) {
    if ( util.is_string(data) )
        data = util.string_to_bytes(data);

    if ( util.is_buffer(data) )
        data = new Uint8Array(data);

    if ( !util.is_bytes(data) )
    ...
    while ( dlen > 0 ) {
        wlen = util._heap_write( heap, pos+len, data, dpos, dlen );
        len  += wlen;
        dpos += wlen;
        dlen -= wlen;
...
module.exports = {
    AES: AES,
    AES_set_key: AES_set_key,
    AES_set_iv: AES_set_iv,
    AES_set_padding: AES_set_padding,
    AES_reset: AES_reset,
    AES_Encrypt_process: AES_Encrypt_process,
    AES_Encrypt_finish: AES_Encrypt_finish,
    AES_Decrypt_process: AES_Decrypt_process,
    AES_Decrypt_finish: AES_Decrypt_finish
}